...
@WebServlet(urlPatterns = "/DisplayAssignments", loadOnStartup = 2)
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (int j = 0; j < assignments.size(); j++) {
				rs = stmt.executeQuery("select * from submissions s where assignment_id=" + assignments.get(j).getId() + ";");
...
//		System.out.println(courses.get(0).getId() +" id");
		...
		request.getRequestDispatcher("/WEB-INF/DisplayAssignments.jsp").forward(request, response);
	}
